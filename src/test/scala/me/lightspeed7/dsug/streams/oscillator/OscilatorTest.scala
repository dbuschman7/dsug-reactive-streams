package me.lightspeed7.dsug.streams.oscillator
...
class ReactiveStreamGraphTest extends FunSuite with BeforeAndAfterAll {
  ...
  test("Create a Reactive Stream in a Graph Shape") {
    ...
    val graph = GraphDSL.create(printSink(1), printSink(2))((m, _) => m) { implicit builder => (sink1, sink2) =>
